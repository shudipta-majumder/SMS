...
export default function LeaveApplicationTable({ leavetransation }) {
  const columns = [
  ...
  function createData(startDate, endDate, Duration, LeaveType, reason, Status) {
    return { startDate, endDate, Duration, LeaveType, reason, Status };
  }

  const rows = leavetransation?.map((leaveapp, key) => {
    return createData(
      leaveapp.start_date || "N/A",
      leaveapp.end_date || "N/A",
      leaveapp.duration || "N/A",
      leaveapp.LeaveType || "N/A",
      leaveapp.reason || "N/A",
      leaveapp.status || "N/A"
    );
  });
