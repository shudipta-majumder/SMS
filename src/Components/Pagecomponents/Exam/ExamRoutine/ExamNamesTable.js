...
const ExamNamesTable = ({ session, ExamNamesList }) => {
  ...
  const ExamNamesSubMenu = academicMenu?.sub_menu
    ...
    : null;
  const ExamRoutinePermissions = ExamNamesSubMenu?.permission || [];
  ...
  useEffect(() => {
    fetch(
      `${process.env.NEXT_PUBLIC_HOST}/exam/api/exam-routine?page_number=${
        page + 1
  ...
  const rows = ExamNamesData?.data.map((item, index) => {
    const permissions = ExamRoutinePermissions || [];
    ...
    swalWithMuiButtons
      ...
      .then((result) => {
        if (result.isConfirmed) {
          fetch(
            `${process.env.NEXT_PUBLIC_HOST}/exam/api/exam-routine/delete/${id}`,
            {
              method: "PATCH",
              headers: {
                Authorization: `Bearer ${accessToken}`,
  ...
  const filteredRows = (sortedRows || []).filter((row) => {
    return row.exam.toLowerCase().includes(searchText.toLowerCase());
  });
                            ...
                            <>
                              {
                                <>
                                  ...
                                  row.permissions.includes("update") ? (
                                    <Link
                                      href={`/exam/exam-routine/update?q=${row.ids}`}
                                    >
                                      <IconButton
                                        onClick={() => {
                                          handleOpen();
