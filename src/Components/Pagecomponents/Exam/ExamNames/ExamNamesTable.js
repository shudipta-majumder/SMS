...
const columns = [
  ...
  { id: "examnames", label: "Exam Names", align: "center", sortable: true },
  { id: "session", label: "Session", align: "center", sortable: true },
  { id: "slno", label: "Sl No", align: "center", sortable: true },
  {
    id: "action",
    label: "Action",
    align: "center",
...
const ExamNamesTable = ({ session, SessionDataList, accessToken, menuData }) => {
  ...
  const { isExamNamesSaved, resetExamNamesSaved } = useExamNamesContext();
  const router = useRouter();
  const [ExamNamesData, setExamNamesData] = useState(null);
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [orderBy, setOrderBy] = useState(null);
  const [order, setOrder] = useState("asc");
  const [editingRow, setEditingRow] = useState(null);
  const [editedExamNamesName, setEditedExamNamesName] = useState("");
  const [beforeExamNamesName, setBeforeExamNamesName] = useState(null);
  ...
  const ExamNamesSubMenu = academicMenu?.sub_menu
    ? academicMenu.sub_menu.find((subMenu) => subMenu.name === "Exam Name")
    : null;
  ...
  useEffect(() => {
    fetch(
      `${process.env.NEXT_PUBLIC_HOST}/exam/api/exam-name?page_number=${
        page + 1
  ...
  }, [deleteTrigered, page, rowsPerPage, editingRow, isExamNamesSaved]);

  const rows = ExamNamesData?.data.map((item, index) => {
    const permissions = ExamNamesPermissions || [];
  ...
  const handleDelete = (id, deletedExamNamesName) => {
    swalWithMuiButtons
      .fire({
        title: "Are you sure?",
        html: `You won't be able to revert <b>${deletedExamNamesName}!</b>`,
        icon: "warning",
      ...
      .then((result) => {
        if (result.isConfirmed) {
          fetch(`${process.env.NEXT_PUBLIC_HOST}/exam/api/exam-name/delete/${id}`, {
            method: "PATCH",
            headers: {
              Authorization: `Bearer ${accessToken}`,
            ...
            .then((responseData) => {
              if (responseData.code == 200) {
                const filterdData = ExamNamesData.data.filter(
                  (ExamNames) => ExamNames.id !== id
                );
                setExamNamesData({ ...ExamNamesData, data: filterdData });
                setDeleteTrigered(true);
                swalWithMuiButtons.fire(
                  "Deleted!",
                  `ExamNames <b>${deletedExamNamesName}</b> has been deleted.`,
                  "success"
                );
              }
              if (responseData.code == 401) {
  ...
  const filteredRows = (sortedRows || []).filter((row) => {
    return row.examnames.toLowerCase().includes(searchText.toLowerCase());
  });
                    ...
                    <StyledTableRow tabIndex={-1} key={row.ids}>
                      {columns.map((column) => (
                        <StyledTableCell key={column.id} align={column.align}>
                          {column.id === "ExamNamesname" &&
                          editingRow === row.ids ? (
                            <TextField
                              width="200px"
                              size="small"
                              value={editedExamNamesName}
                              onChange={(e) =>
                                setEditedExamNamesName(e.target.value)
                              }
                              sx={{
                                border: "none",
                                "& input": {
                              ...
                              {editingRow === row.ids ? (
                                ...
                                <>
                                  ...
                                  row.permissions.includes("delete") ? (
                                    <IconButton
                                      onClick={(e) =>
                                        handleDelete(row.ids, row.ExamNamesname)
                                      }
                                    >
                                      <Icon
                                        className="text-rose-700"
